...
# DBSCAN: required specific classes
...
scenario_ids = dataframe['scenario'].astype(int).tolist()
...
# MIN_SAMPLES >= data_dimensions + 1

############################################################################################
# DBSCAN ###################################################################################
############################################################################################
# Agorithm execution
dbscan = DBSCAN(eps=0.115,min_samples=7).fit(X)
...
print(dataframe)
copy['index' ] =dataframe['index'].values
copy['scenario' ] =dataframe['scenario'].values
copy['label'] = labels
