...
filename ="Scenarios-APStats.csv"
...
df.to_csv(r"Scenarios-APStats-scaled.csv")
...
############################################################################################
# Component range
component_quantity = range(1, 10)

# Centroid distance fixing method
models = [GaussianMixture(n).fit(X) for n in component_quantity]

# Curve display (AIC, BIC)
plt.rcParams['figure.figsize'] = (10, 10)
plt.plot(component_quantity, [m.bic(X) for m in models], label='BIC')
plt.plot(component_quantity, [m.aic(X) for m in models], label='AIC')
plt.xlabel('Number of Components', fontsize=22)
plt.ylabel('Score',fontsize=22)
plt.legend(loc='best',fontsize=22)
plt.title('Akaike and Bayesian information criterions',fontsize=22)
plt.show()
############################################################################################
# GMM ######################################################################################
############################################################################################
# Clustering settings
gmm = GaussianMixture(n_init=10,n_components=4,max_iter=100).fit(X)
